...
def prepare_common_run(config: Dict[str, Any], run_name: str) -> Tuple[Path, pd.DataFrame, list, pd.DataFrame]:
    ...
    setup_logging(
        ...
        console_mode="compact" if config.get("compact_progress", False) else "standard",
    )
    logger = get_logger("experiments.common.runner")
    logger.info("Run avviata: %s", run_name, extra={"console": True})
    logger.info("Dataset: %s", config["dataset"], extra={"console": True})
    logger.info("Soglie grafo: min_nodes=%s min_edges=%s", config.get("min_nodes", 3), config.get("min_edges", 2), extra={"console": True})
    df = load_dataset(
        config["dataset"],
        max_repositories=config.get("max_repositories"),
        max_samples=config.get("max_samples"),
    )
    logger.info("Dataset caricato: %s righe, %s repository", len(df), df["repository"].nunique(), extra={"console": True})
    ...
    logger.info("Campioni comuni mantenuti: %s; esclusi: %s", len(df), len(excluded), extra={"console": True})
    ...
    logger.info("Split walk-forward validi: %s; split saltati: %s", len(splits), len(skipped), extra={"console": True})
    ...
    logger.info("Manifest salvati in: %s", run_dir, extra={"console": True})
